...
function fmtM(n: number) { return `${toAr((n / 1_000_000).toFixed(1))}م` }
...
function RevenueTrendChart({ factor, revenueTrend }: { factor: number; revenueTrend: RevenueTrend[] }) {
  const data = useMemo(() => {
    const count = factor <= 0.25 ? 7 : factor <= 1 ? revenueTrend.length : revenueTrend.length
...
function FunnelPanel({ factor, funnelStages }: { factor: number; funnelStages: FunnelStage[] }) {
  const data = useMemo(() => funnelStages.map((s) => ({ ...s, count: Math.round(s.count * factor) })), [factor, funnelStages])
            ...
            <div key={stage.stage} className="flex items-center gap-2">
              ...
              <div className="flex-1 h-6 bg-muted rounded-full overflow-hidden">
                <div className="h-full rounded-full flex items-center justify-end pe-2 transition-all duration-500"
                  ...
                  <span className="text-white text-[11px] font-bold font-inter">{toAr(stage.count)}</span>
                </div>
              </div>
              <span className="w-8 text-[11px] text-muted-foreground shrink-0 font-inter">{toAr(pct)}٪</span>
            </div>
          )
        })}
      </div>
...
export function ReportsClient({ kpiData, funnelStages, channelPerformance, revenueTrend, salesReps }: ReportsClientProps) {
  ...
  const kpi = useMemo(() => ({
    ...
    revenue: Math.round(kpiData.revenueThisMonth * factor),
