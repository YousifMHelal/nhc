...
import { LEADS, LEAD_SCORES, SALES_REPS } from '@/lib/mock-data'
import type { Lead, LeadScore, LeadGrade } from '@/lib/types'
...
function ScoreDetailPanel({ lead, score }: { lead: Lead; score: LeadScore }) {
  const rep = SALES_REPS.find((r) => r.id === lead.salesRepId)
...
const SCORED_LEAD_IDS = new Set(['lead-015', 'lead-013', 'lead-006', 'lead-002', 'lead-003'])
const VISIBLE_STAGES = ['New', 'Contacted', 'Qualified', 'Proposal']

export default function LeadScoringPage() {
  const scorableLeads = LEADS.filter((l) => VISIBLE_STAGES.includes(l.stage))
  ...
  const selectedScore = LEAD_SCORES.find((s) => s.leadId === selectedId)
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
          ...
          <div className="flex flex-col gap-2 overflow-y-auto max-h-[600px]">
            {scorableLeads.map((lead) => (
              <LeadListItem
                key={lead.id}
                lead={lead}
                score={LEAD_SCORES.find((s) => s.leadId === lead.id)}
        ...
        <div className="lg:col-span-2">
          {selectedLead && selectedScore ? (
            <ScoreDetailPanel lead={selectedLead} score={selectedScore} />
          ) : selectedLead ? (
