...
import { LEADS, SALES_REPS } from '@/lib/mock-data'
import type { Lead, PipelineStage, LeadSource, Channel } from '@/lib/types'
...
function AddLeadDialog({
  ...
  const [form, setForm] = useState({
    ...
    city: 'الرياض',
    budget: '',
    salesRepId: SALES_REPS[0]?.id ?? 'rep-001',
  })
  ...
  const handleSubmit = () => {
    ...
    setForm({
      ...
      propertyInterest: 'فيلا', city: 'الرياض', budget: '', salesRepId: SALES_REPS[0]?.id ?? 'rep-001',
    })
    onClose()
  }
      ...
      <DialogContent className="sm:max-w-lg" showCloseButton>
        ...
        <div className="grid grid-cols-2 gap-3 py-2">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            >
              {SALES_REPS.map((r) => <option key={r.id} value={r.id}>{r.nameAr}</option>)}
            </select>
          </div>
        </div>
...
export default function PipelinePage() {
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      // If dropped on a card rather than a column, find that card's column
