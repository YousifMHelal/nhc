...
export function Customer360Client({ customers, allTimeline, allOpportunities, allContracts }: Props) {
  ...
  const timeline = allTimeline.filter((e) => e.customerId === customer?.id)
  const opportunities = allOpportunities.filter((o) => o.customerId === customer?.id)
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="w-56 relative">
          <select
            ...
            value={customerId}
            onChange={(e) => { setCustomerId(e.target.value); setActiveFilter('') }}
          >
            {customers.map((c) => (
